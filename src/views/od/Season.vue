    ...
    <fieldset>
      ...
      <table>
        ...
        <tbody>
            ...
            <tr>
              <th>시즌명</th>
              <td colspan="3"><input type="text" v-model="search.seasonName"/></td>
            </tr>
        </tbody>
...
import SeasonService from '@src/service/od/SeasonService';
...
const getList = (event: Event) => {
  ...
  data.totalCount = 0;
  SeasonService.getSeasonList(search).then(
    (res: any) => {
...
const save = () => {
  ...
  SeasonService.setSeasonList(saveList).then(() => {
    getList(); // 저장 후 목록 갱신
  });
};
onMounted(() => {
