...
class ApiHistoryService {
	...
	async insertApiHistory(formData:FormData) { // API이력 등록 [BS_API_HIST]
		const res = await apiConfig('/bo/cr/insertApiHistory', formData);
		return res.data;
	}
	async updateApiHistory(formData:FormData) { // API이력 수정 [BS_API_HIST]
		const res = await apiConfig('/bo/cr/updateApiHistory', formData);
		return res.data;
	}
	async deleteApiHistory(formData:FormData) { // API이력 삭제 [BS_API_HIST]
		const res = await apiConfig('/bo/cr/getApiHistoryList', formData);
		return res.data;
	}
}
