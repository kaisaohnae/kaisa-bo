...
export default function CompanyPage() {
  ...
  const getList = async () => {
    CompanyService.getCompanyList({ ...search, page: data.currentPage }).then(res => {
      setData(prev => ({
        ...prev,
        list: res.data?.list || [],
  ...
  const save = async () => {
    ...
    if (!saveList) return;
    CompanyService.setCompanyList(saveList).then(async () => {
      await getList();
    });
  };
      ...
      >
        <fieldset>
          ...
          <table>
            ...
            <tr>
              <th scope="row">업체명</th>
              <td colSpan={3}><input type="text" value={search.companyName} onChange={e => handleSearchChange('companyName', e.target.value)} /></td>
            </tr>
            </tbody>
            {data.audit && (
        ...
        <div className="buttons">
          ...
          <button type="button" className="button excel" onClick={() => excelUtil.excelExport(handsontable.current, '업체')}>
