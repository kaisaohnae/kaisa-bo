...
export default function CompanyPage() {
  ...
  const save = () => {
    ...
    if (!saveList) return;
    CompanyService.setCompanyList(saveList).then(() => getList());
  };
  ...
  return (
    <>
      ...
      >
        <fieldset>
          ...
          <table>
            ...
            <tr>
              <th scope="row">업체명</th>
              <td colSpan={3}><input type="text" value={search.companyName} onChange={e => handleSearchChange('search.companyName', e.target.value)} /></td>
            </tr>
            </tbody>
            {data.audit && (
      ...
      <button type="button" className="audit" onClick={() => setData(prev => ({...prev, audit: !prev.audit}))}>상세조회</button>
          ...
          <button type="button" className="button excel" onClick={() => excelUtil.excelExport(data.grid, '업체')}>
