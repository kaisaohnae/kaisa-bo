...
import VersionService from '@/service/ap/version-service';
...
export default function VersionPage() {
  ...
  const getList = async () => {
    VersionService.getVersionList({ ...search, page: data.currentPage }).then(res => {
      setData(prev => ({
        ...prev,
        list: res.data?.list || [],
  ...
  const save = async () => {
    ...
    if (!saveList) return;
    VersionService.setVersionList(saveList).then(async () => {
      await getList();
    });
  };
  ...
  useEffect(() => {
    ...
    handsontable.current = new Handsontable(gridRef.current, {
      ...
      {data: 'appId', type: 'text', width: 150, readOnly: true,  },
      {data: 'version', type: 'text', width: 150, readOnly: true,  },
      {data: 'applyDt', type: 'date', width: 170,   ...gridUtil.dateTimePickerConfig },
      {data: 'isForce', type: 'dropdown', width: 150,   source: function (query, process) { process(setting.codeList['isForce']?.map((o: any) => o.codeValue)) }},
      {data: 'osCode', type: 'dropdown', width: 150, readOnly: true,  source: function (query, process) { process(setting.codeList['osCode']?.map((o: any) => o.codeValue)) }},
    ...
    <>
      ...
      >
        ...
        <div className="buttons">
          <CrudButtons menuId="version" add={add} del={del} save={save} />
          ...
          <button type="button" className="button excel" onClick={() => excelUtil.excelExport(handsontable.current, '버전')}>
