...
import ManagerService from '@/service/cs/manager-service';
...
export default function ManagerPage() {
  ...
  const getList = async () => {
    ManagerService.getManagerList({ ...search, page: data.currentPage }).then(res => {
      setData(prev => ({
        ...prev,
        list: res.data?.list || [],
  ...
  const save = async () => {
    ...
    if (!saveList) return;
    ManagerService.setManagerList(saveList).then(async () => {
      await getList();
    });
  };
  ...
  useEffect(() => {
    ...
    handsontable.current = new Handsontable(gridRef.current, {
      ...
      {data: 'managerNo', type: 'numeric', width: 150, readOnly: true,  },
      {data: 'companyId', type: 'text', width: 100, readOnly: true,  },
      {data: 'managerName', type: 'text', width: 150,   },
    ...
    <>
      ...
      >
        <div className="field">
          <table>
            ...
            <tr>
              ...
              <td colSpan={3}><input type="text" value={search.managerName} onChange={e => handleSearchChange('managerName', e.target.value)} /></td>
            </tr>
            <tr>
        ...
        <div className="buttons">
          <CrudButtons menuId="manager" add={add} del={del} save={save} />
          ...
          <button type="button" className="button excel" onClick={() => excelUtil.excelExport(handsontable.current, '담당자')}>
