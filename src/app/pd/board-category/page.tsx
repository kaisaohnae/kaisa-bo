...
import BoardCategoryService from '@/service/pd/board-category-service';
...
export default function BoardCategoryPage() {
  ...
  const getList = async () => {
    BoardCategoryService.getBoardCategoryList({ ...search, page: data.currentPage }).then(res => {
      setData(prev => ({
        ...prev,
        list: res.data?.list || [],
  ...
  const save = async () => {
    ...
    if (!saveList) return;
    BoardCategoryService.setBoardCategoryList(saveList).then(async () => {
      await getList();
    });
  };
  ...
  useEffect(() => {
    ...
    handsontable.current = new Handsontable(gridRef.current, {
      ...
      {data: 'boardCategoryId', type: 'numeric', width: 150, readOnly: true,  },
      {data: 'companyId', type: 'text', width: 100, readOnly: true,  },
      {data: 'categoryName', type: 'text', width: 150,   },
    ...
    <>
      ...
      >
        <div className="field">
          <table>
            ...
            <tr>
              ...
              <td colSpan={3}><input type="text" value={search.categoryName} onChange={e => handleSearchChange('categoryName', e.target.value)} /></td>
            </tr>
            </tbody>
            {data.audit && (
        ...
        <div className="buttons">
          <CrudButtons menuId="pd-board-category" add={add} del={del} save={save} />
          ...
          <button type="button" className="button excel" onClick={() => excelUtil.excelExport(handsontable.current, '게시판카테고리')}>
