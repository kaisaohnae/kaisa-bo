...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    handsontable.current = new Handsontable(gridRef.current, {
      ...
      {data: 'productNo', type: 'numeric', width: 90, readOnly: true,  },
      {data: 'companyId', type: 'text', width: 100, readOnly: true,  },
      {data: 'seasonPriceNo', type: 'numeric', width: 100,   },
      {data: 'productName', type: 'text', width: 100,  className: 'underline', },
      {data: 'headCount', type: 'numeric', width: 100,   },
      {data: 'maxHeadCount', type: 'numeric', width: 100,   },
      {data: 'm2', type: 'numeric', width: 100,   },
      {data: 'isDisplay', type: 'dropdown', width: 80,   source: function (query, process) { process(setting.codeList['isDisplay']?.map((o: any) => o.codeValue)) }},
      {data: 'isPet', type: 'dropdown', width: 80,   source: function (query, process) { process(setting.codeList['isPet']?.map((o: any) => o.codeValue)) }},
      {data: 'isBBQ', type: 'dropdown', width: 80,   source: function (query, process) { process(setting.codeList['isBBQ']?.map((o: any) => o.codeValue)) }},
      {data: 'isPickup', type: 'dropdown', width: 80,   source: function (query, process) { process(setting.codeList['isPickup']?.map((o: any) => o.codeValue)) }},
      {data: 'isStone', type: 'dropdown', width: 80,   source: function (query, process) { process(setting.codeList['isStone']?.map((o: any) => o.codeValue)) }},
      {data: 'memo', type: 'text', width: 100,   },
      {data: 'fileNo', type: 'numeric', width: 100,   },
    ...
    <>
      ...
      >
        ...
        <div className="buttons">
          <CrudButtons menuId="pd-product" add={add} del={del} save={save} />
      ...
      {detailShow && (
       <>
         <Detail component={ProductDetail} detailData={detailData} detailShow={detailShow} setDetailShow={setDetailShow} getList={getList} />
       </>
      )}
