...
export default function ProductPage() {
  ...
  const save = () => {
    ...
    if (!saveList) return;
    ProductService.setProductList(saveList).then(() => getList());
  };
  ...
  return (
    <>
      ...
      >
        <fieldset>
          ...
          <table>
            ...
            <tr>
              <th scope="row">상품명</th>
              <td colSpan={3}><input type="text" value={search.productName} onChange={e => handleSearchChange('search.productName', e.target.value)} /></td>
            </tr>
            </tbody>
            {data.audit && (
      ...
      <button type="button" className="audit" onClick={() => setData(prev => ({...prev, audit: !prev.audit}))}>상세조회</button>
          ...
          <button type="button" className="button excel" onClick={() => excelUtil.excelExport(data.grid, '상품')}>
